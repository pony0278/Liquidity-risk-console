...
- **資料太舊會退出機器判斷。** 過期的指標仍顯示最後數值供人工參考，但燈號會
  變成「資料過期」，Tier、引信、傳導鏈與擁擠層一律視為 unknown；排程也回報
  資料缺口。門檻跟著各指標宣告的 `freq` 走（每日 7 天、每週 14、每月 75），
...
完整測試完全離線：用合成的「平靜」與「壓力」兩套歷史跑完整條管線，
...
本工具的定位是辨認壓力停留在「價格／估值」、已擴散到「信用」，或進一步
走到「資金融通」；它不是買賣時點或報酬預測模型。傳導鏈只計算由前到後連續
成立的動態節點，人工維護的結構背景顯示為 BACKGROUND／ARMED，不計入進度。
後段訊號若提早出現，只當作旁證，不得跳過中間節點把鏈條判成已推進。

目前累積歷史仍短，設定門檻屬風險監測假說，尚未通過涵蓋多次危機的逐日歷史
回放；在完成無前視偏誤的回測前，不應單獨用來決定加減倉、槓桿或抄底。
